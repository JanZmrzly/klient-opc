...
from PyQt5.QtWidgets import QApplication, QAbstractItemView, QAction, QTreeView
...
class TreeWidget(QObject):
    ...
    def resizeColumnToContents(self, column):
        self.model.resizeColumnToContents(column)
...
class TreeViewModel(QStandardItemModel,QTreeView):
    ...
    def _resizeColumnToContents(self, column):
    ...
    def add_item(self, desc, parent=None, node=None):
        ...
        #item = [QStandardItem(bname), QStandardItem(dname), QStandardItem(nodeid)]
    
        item = [QStandardItem(bname), QStandardItem(nodeid)]
        if desc.NodeClass == ua.NodeClass.Object:
            if desc.TypeDefinition == ua.TwoByteNodeId(ua.ObjectIds.FolderType):
                item[0].setIcon(QIcon("OpcUaClient_mainwindowUI/icones_for_gui/folder.png"))
                # item[0].setIcon(QIcon(":/folder.svg"))
            else:
                item[0].setIcon(QIcon("OpcUaClient_mainwindowUI/icones_for_gui/node.png"))
            ...
            if desc.TypeDefinition == ua.TwoByteNodeId(ua.ObjectIds.PropertyType):
                ...
                item[0].setIcon(QIcon("OpcUaClient_mainwindowUI/icones_for_gui/variable.png"))
